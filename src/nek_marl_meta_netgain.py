"""
MARL ENV using NEK5000 (meta-evaluation)
@yuningw
"""
...
class parallel_env(ParallelEnv):
    ...
    def initialization(self):
        ...
        # Scaling the data
        self.utau = 0.061  # u_tau for wing @ x/c =  0.4
        # -----------ACTION--------------
        # Gll node weight
    ...
    def evolve(self):
        ...
        while i_evolv <= (self.conf.simulation.ndrl):
            ...
            if i_evolv == self.conf.simulation.ndrl:
                TOTCTRL = self.conf.simulation.TOTCTRL
                if self.reward_fn == 'net_gain':
                    # net_gain needs three raw buffers: wall-shear, pumping power, v3
                    ...
                    ws_stress_buffer = self._assign_reward(ws_stress_buffer)
        ...
        for il, nid in enumerate(self.uniqID):
            ...
            for jl, gllid in enumerate(self.agent_info['GLLID'][indx]):
                ...
                if self.reward_fn == 'net_gain':
                    r_reward = self._normalize_reward(
                        tau_w=tau_dist[agent_name],
                        pw=pw_dist[agent_name],
                        v3=v3_dist[agent_name],
                    )
                else:
                    r_reward = ws_stress_buffer[agent_name].squeeze()
                    # Do not normalize dUdy here, meta policy handles scaling
                rewards[agent_name] = r_reward
        # Logging reward for debugging and further analysis
        self.reward_log.append(r_reward)
    ...
    def _assign_reward(self, reward_buffer: np.ndarray):
        ...
        if self.conf.runner.rew_mode == 'Homo':
            ...
            reward_buffer = np.ones_like(reward_buffer) * np.mean(reward_buffer.flatten())
