    ...
    def evolve(self):
        ...
        for il, nid in enumerate(self.uniqID): 
            ...
            for jl, gllid in enumerate(self.agent_info['GLLID'][indx]):
                ...
                ## NOTE: Do not normalize the dUdy at this stage, it will be normalized later
                #### r_reward   = self._normalize_reward(r_reward)
                rewards[agent_name] = r_reward
        # Logging reward for debugging and further analysis
        self.reward_log.append(r_reward)
        # print(f"[LOGGER] dUdy={r_reward:.5f} R={r_reward:.5f} ",flush=True)
...
#####################
    ...
    def avg_ZNMF(self,ctrl_value:dict):
        ...
        # ## Uniform blowing
        # elif self.conf.simulation.znmf_avg==-3:
        #     for agent_name in ctrl_value.keys():
        #         ctrl_value[agent_name]= -1 * self.ctrl_max_amp
        #         # print(f"[ZNMF] {agent_name}:\n BEF={ctrl_value_single},NOW={ctrl_value_znmf}",flush=True)
