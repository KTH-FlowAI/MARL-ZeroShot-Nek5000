...
#-------------------------------------------
  def run(self):
    """
    Main Programe for meta-policy exploitation
    """
...
#--------------------------------------------
  def _partial_predict(self,case_name:str,observation,state,
    ...
    if if_update: 
      # If requries update, we renew the buffer 
      ## Get the observation 
      ...
      #### XXX: Do we really need old observation buffer?
      ### self.policy_dict[case_name]['partial_obs'] = partial_obs
      ...
      #### XXX: Do we really need old observation buffer? 
      ### partial_obs = deepcopy(self.policy_dict[case_name]['partial_obs'])
