...
c=============================================
        ...
        ! The state is the local wall-normal velocity only.
        real ctrlVn(LX1,LY1,LZ1,LELT)
        integer nfail 
...
c---------------------------------
cc STEP 1: Project Cartesian velocity onto the wall-normal direction.
        call normal_V_project(ctrlVn)
c----------------------------------
...
c--------------------------------------------------------------------
        subroutine normal_V_project(ctrlVn)
cc: Project Cartesian velocity onto the wall-normal direction.
c=============================================
c       Define variable
c=============================================
        implicit none 
...
c=============================================
        ...
        ! The sole observation is the wall-normal velocity V_n.
        call rzero(rotV,ntot)
        rotV=vx*body_cos + vy*body_sin
        call copy(ctrlVn(1,1,1,1),rotV(1,1,1,1),ntot)
...
c        !Write down the face normal 
        if (ISTEP.eq.1) then
        call outpost(ctrlVn(1,1,1,1),ctrlVn(1,1,1,1),vx,vy,t,'rot')
...
c--------------------------------------------------------------------
        subroutine vel_pts_find(ctrlVn)
c: Sample the wall-normal velocity at the sensing-plane points.
c=============================================
c       Define variable
c=============================================
        implicit none 
        ...
        ! Wall-normal velocity fluctuation field
        real ctrlVn(LX1,LY1,LZ1,LELT)
        ...
        character*4 str, str1
        real ctrlVn_test(LX1,LY1,LZ1,LELT)
#endif
c=============================================
c       Function
c=============================================
        ...
        call outpost(ctrlVn,ctrlVn,vx,vy,t,'flu')
        endif
#endif
!     Interpolate the only state field, wall-normal v'.
        ! Use a flatten buffer to ensure the interpolation.
        call rzero(buffV(1),ntot)
        call copy(buffV,ctrlVn(1,1,1,1),ntot)

        ! The third argument is the number of packed fields (one).
        call findpts_eval(inth_hpts1,val_obs(1,1),NFLDC,
     &                       rcode,1,
     &                       proc,1,
     &                       elid,1,
     &                       rst,NDIM,NUMCTRL,
...
c$$$ TEST: Write down all the sensing points that we have found and the averaged velocity
     ...
     $     (val_obs(ily,ilx), ily = 1,nfldc)
        ...
        ! Examine the interpolated velocity
         ...
         ily = info_agt(4,il)
         ilz = info_agt(5,il)
         ctrlVn_test(ilx,ily,ilz,iel) = val_obs(1,il)
         enddo
         call outpost(ctrlVn_test,ctrlVn_test,ctrlVn,ctrlVn,t,'fli')
         endif
#endif
c$$$ TEST END        
        return 
