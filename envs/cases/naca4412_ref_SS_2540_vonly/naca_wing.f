...
c     set up user-defined boundary conditions
      ...
      include 'DRL'
      real Vb 
      parameter(Vb=-0.000) ! 0.2% of uniform suction 
#endif 
...
c-------------------------------------------------
      ...
      if (y.gt.0) then
      ux = vf*snx ! Control on reference 
      uy = vf*sny ! Control on reference
      else ! On the Pressure side the sign must be flipped if we use the mask
      ux = 1.0*vf*snx 
      uy = 1.0*vf*sny 
      endif 
