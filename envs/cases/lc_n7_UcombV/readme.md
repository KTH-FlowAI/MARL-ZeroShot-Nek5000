...
## OC_UVCOMB state contract

`lc_n7_UcombV` is the two-state channel case for the embedded `OC_UVCOMB`
analytic policy. Its state and recorder order are `[u', v']` at the selected
sensing plane. `OC_UVCOMB` imposes `-alpha * (0.5*u' - v')`; build the
Python-free solver with `bash ./compile_script --solver raw --all`.
