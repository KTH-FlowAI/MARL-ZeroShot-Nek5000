...
!-----------------------------------------------------------------------
     ...
     $        dwdx(1,1,1,1,1),itmp)
      ...
      ! [MOD] Disabled pressure normalization: Dirichlet walls have no "W"
      ! [MOD] faces, so vrtmp2(1) is zero and the division produces NaN.
      ! [MOD] ! normalise pressure
      ! [MOD] ! in this example I integrate pressure over top faces marked "W"
      ! [MOD] ifll = 1     ! I'm interested in velocity bc
      ! [MOD] ! relying on mesh structure given by genbox set face number
      ! [MOD] jl = 3
      ! [MOD] call rzero(vrtmp2,2)  ! zero work array
      ! [MOD] itmp = LX1*LZ1
      ! [MOD] do il=1,nelv   ! element loop
      ! [MOD]    if (cbc(jl,il,ifll).eq.'W  ') then
      ! [MOD]       vrtmp2(1) = vrtmp2(1) + vlsum(area(1,1,jl,il),itmp)
      ! [MOD]       call ftovec(vrtmp,pres,il,jl,lx1,ly1,lz1)
      ! [MOD]       call col2(vrtmp,area(1,1,jl,il),itmp)
      ! [MOD]       vrtmp2(2) = vrtmp2(2) + vlsum(vrtmp,itmp)
      ! [MOD]    endif
      ! [MOD] enddo
      ! [MOD] ! global communication
      ! [MOD] call gop(vrtmp2,vrtmp,'+  ',2)
      ! [MOD] ! missing error check vrtmp2(1) == 0
      ! [MOD] vrtmp2(2) = -vrtmp2(2)/vrtmp2(1)
      ! [MOD] ! remove mean pressure
      ! [MOD] itmp = LX1*LY1*LZ1*NELV
      ! [MOD] call cadd(pres,vrtmp2(2),itmp)
