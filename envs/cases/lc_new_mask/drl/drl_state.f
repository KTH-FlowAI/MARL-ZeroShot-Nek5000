...
c=============================================
        ...
        call copy(ctrlV(1,1,1,1,1),vx(1,1,1,1),ntot)
        ! The 2nd is the Wall-normal velocity 
        call copy(ctrlV(2,1,1,1,1),vy(1,1,1,1),ntot)
...
c        !Write down the face normal 
        if (ISTEP.eq.2) then
        call outpost(ctrlV(1,1,1,1,1),ctrlV(2,1,1,1,1),vx,pr,t,'ang')
...
c-----------------------------------------------
        ...
        call copy(wrk_buff(1,1,1,1),ctrlV(fl,1,1,1,1),ntot)
...
c-----------------------------------------------
     ...
     $                      - velV(:,:,:,:)
        ...
        call copy(ctrlV(fl,1,1,1,1),wrk_buff(1,1,1,1),ntot)

        enddo ! do fl=1,NFLDC
        if(NID.eq.0) print *, "[DRL] STATE AVG SUBSTRACT!"
        else   ! if one does not do any averaging
        goto 119 
        endif ! if (obs_xavg .or. obs_zavg)
c-----------------------------------------------
119     return
...
!     Velocity interpolation 
        ...
        call fgslib_findpts_eval(inth_hpts1,val_obs(il,1),NFLDC,
     &                       iwk(1,1),1,
     &                       iwk(1,3),1,
     &                       iwk(1,2),1,
