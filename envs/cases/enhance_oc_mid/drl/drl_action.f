...
c=============================================
      ...
      ! it should be just value between -1 to 1 
      call copy(velV(1,1,1,1),ACTIONS(1,1,1,1),ntot)
      call dssum(velV,lx1,ly1,lz1)
      ! Impose the amplitude to vertical velocity
      do il=1,numctrl
        iel = info_agt(1,il)
        iel = gllel(iel)
        ix  = info_agt(3,il)
        iy  = info_agt(4,il)
        iz  = info_agt(5,il)
        wrk_buff(ix,iy,iz,iel)=val_obs(2,il)
      enddo 
      ! Fill the holes by vertical vel
      call dssum(wrk_buff,lx1,ly1,lz1)
      ! act = \alpha * v_vel 
      call cfill(diff_buff, -1.0, ntot)
      call col2(wrk_buff,diff_buff,ntot)
      ! actV = act_oc + act_drl 
      call add3(actV, wrk_buff, velV, ntot)
      call copy(velV,actV,NTOT)
      
#ifdef YWDEBUG  
      if (ISTEP.eq.1) then 
      call outpost(velV,wrk_buff,actV,vy,t,'dss')
      ...
      enddo
      ! Subtract the mean again of the action 
