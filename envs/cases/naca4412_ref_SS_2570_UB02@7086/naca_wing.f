...
c===================================
#ifdef BLOW 
! Impose uniform blowing at some region at the same time. 
! I hard coded it here but later one can customize it
      if (x.ge.xmin.and.x.le.xmax.and.
     $    y.ge.0.0.and.y.le.0.6) then 
...
cc MA:
      ...
      include 'GEOM'
      integer e,f,fmid(6)
      real xmin,xmax,xf,yf,zf
      integer im, jm, km, nface
#endif 
...
#ifdef BLOW
cc RV: needed for blowing:
      if (nid.eq.0) write(*,*) 'INITIALIZTION OF BLOWING'
...
!     Minimum and maximum x coordinates to apply blowing
      xmin = 0.69 ! 
      xmax = 0.86 ! 
...
!     Then x coordinate of face center must be between xmin and xmax, and
!     be a point from the suctions side of the wing
            
            if (cbc(f,e,1).eq.'W  '
     &           .and.xf.ge.xmin .and. xf.le.xmax .and. yf.gt.0.0) then
               cbc(f,e,1)='v  ' ! Change Wall BC to Dirichlet
           endif
        enddo
      enddo
      if (NID.eq.0) print*,"BLOW AREA:",xmin,xmax
#endif  
